...
class MessageHandler(
    ...
    fun verifySignature(secret: String, signature: String, body: String): Boolean {
    ...
    fun handle(event: Event, appCode: String) {
        when (event) {
            is MessageEvent -> handleMessageEvent(event, appCode)
            else -> {
            }
        }
    }

    private fun handleMessageEvent(event: MessageEvent, appCode: String) {
        ...
        if (!isAuthenticated) {
            tryAuthenticate(id, text, appCode)
            return
        }
        when (text) {
            "resume" -> resume(id)
    ...
    private fun tryAuthenticate(id: String, text: String, appCode: String) {
        if (appCode != text) {
            return
        }
